...
public class PrefixTree
{
    ...
    public PrefixTree(string prefix, int prefixNumberOfDigits, string agency, IEnumerable<RangeRule> rules)
    {
        ...
        //Rules with most digits first; 'greedy'
        var greedyDigitComparer = Comparer<RangeRule>.Create((x, y) => y.NumberOfDigits.CompareTo(x.NumberOfDigits));
        rulesList.Sort(greedyDigitComparer);

        _rulesSortedByNumberOfDigits = rulesList;
        _rulesCount                  = _rulesSortedByNumberOfDigits.Count;

        var prefixTreeComparer = Comparer<PrefixTree>
            .Create((x, y) =>
                    {
        ...
        _children = new SortedSet<PrefixTree>(prefixTreeComparer);

        _prefixStr = prefix;
    }

    public bool AddChild(string                 groupPrefix
                       , int                    groupPrefixNumberOfDigits
                       , string                 agencyName
                       , IEnumerable<RangeRule> groupRules)
    {
