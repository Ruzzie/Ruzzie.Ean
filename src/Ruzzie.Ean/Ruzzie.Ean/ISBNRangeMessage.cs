...
{
    [XmlElement(ElementName = "EAN.UCC")]
    public List<EAN_UCC>? EAN_UCC { get; set; }
}
...
public class ISBNRangeMessage
{
    ...
    {
        const string resourceName = "Ruzzie.Ean.ISBNRangeMessage.xml";
        try
        {
            ...
            var       deserializedObject = ISBNRangeMessageSerializer.Deserialize(streamReader);
            if (deserializedObject == null)
            {
                return new Err<LoadErrorKind>(
                                              $"Deserialization failed. The deserialized object is null for {resourceName}."
                                            , LoadErrorKind.DeserializedObjectIsNull);
            }

            return (ISBNRangeMessage)deserializedObject;
        }
        catch (Exception e)
        {
            return new Err<LoadErrorKind>(
